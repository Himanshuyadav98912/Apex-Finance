  ...
  useEffect(() => {
    const fetchPortfolio = async () => {
        try {
            ...
            // If initial load fails, portfolio remains null and UI will show error
            // If a poll fails, we'll just log the error and the UI will show stale data.
        } finally {
            // This gate ensures we only turn off the main loading indicator once,
            // preventing UI flickers on subsequent polls.
            if (loading) {
                setLoading(false);
            }
        }
