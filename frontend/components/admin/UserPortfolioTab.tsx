...
const UserPortfolioTab: React.FC = () => {
    ...
    const fetchUsers = useCallback(async () => {
        ...
        try {
            ...
            setError(error.message || "Could not fetch user data.");
        } finally {
            setIsLoading(false);
        }
    }, []);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await fetchUsers();
             ...
             setError(error.message || "Could not update user data.");
        } finally {
    ...
    return (
        <>
            ...
            <div className="bg-slate-800 rounded-xl shadow-lg border border-slate-700 overflow-hidden">
                ...
                {error && (
                    <div className="p-3 mx-4 my-2 text-sm text-red-200 bg-red-800/50 rounded-lg flex justify-between items-center">
                        <span>{error}</span>
                        ...
                        <tbody className="bg-slate-800 divide-y divide-slate-700">
                           {users.map(user => (
                               <tr key={user.id} className="hover:bg-slate-700/50 transition-colors">
                                   ...
                                   <td className="px-3 py-2 whitespace-nowrap">
                                       <input type="text" inputMode="decimal" value={user.totalInvested} onChange={(e) => handleInputChange(user.id, 'totalInvested', e.target.value)}
                                         className="p-2 w-36 bg-slate-700 rounded border border-slate-600 focus:ring-indigo-500 focus:border-indigo-500 text-white" />
                                   </td>
                                   <td className="px-3 py-2 whitespace-nowrap">
                                       <input type="text" inputMode="decimal" value={user.totalReturns} onChange={(e) => handleInputChange(user.id, 'totalReturns', e.target.value)}
                                         className="p-2 w-36 bg-slate-700 rounded border border-slate-600 focus:ring-indigo-500 focus:border-indigo-500 text-white" />
                                   </td>
                                   <td className="px-3 py-2 whitespace-nowrap">
                                        <input type="text" inputMode="decimal" value={user.ytdReturnPercent} onChange={(e) => handleInputChange(user.id, 'ytdReturnPercent', e.target.value)}
                                         className="p-2 w-24 bg-slate-700 rounded border border-slate-600 focus:ring-indigo-500 focus:border-indigo-500 text-white" />
                                   </td>
                                   <td className="px-3 py-2 whitespace-nowrap">
                                        <input type="text" inputMode="decimal" value={user.dayChange} onChange={(e) => handleInputChange(user.id, 'dayChange', e.target.value)}
                                         className="p-2 w-28 bg-slate-700 rounded border border-slate-600 focus:ring-indigo-500 focus:border-indigo-500 text-white" />
                                   </td>
                                   <td className="px-3 py-2 whitespace-nowrap">
                                        <input type="text" inputMode="decimal" value={user.dayChangePercent} onChange={(e) => handleInputChange(user.id, 'dayChangePercent', e.target.value)}
                                         className="p-2 w-24 bg-slate-700 rounded border border-slate-600 focus:ring-indigo-500 focus:border-indigo-500 text-white" />
                                   </td>
                                   <td className="px-3 py-2 whitespace-nowrap">
