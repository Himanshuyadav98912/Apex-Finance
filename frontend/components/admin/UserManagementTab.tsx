...
const UserManagementTab: React.FC = () => {
    ...
    const fetchUsers = useCallback(async () => {
        ...
        try {
            ...
            setError("Could not fetch users. Please try refreshing the page.");
        } finally {
            setIsLoading(false);
        }
    }, []);
    ...
    const handleDeleteSelected = async () => {
        if (selectedUsers.size === 0 || !window.confirm(`Are you sure you want to delete ${selectedUsers.size} selected users?`)) return;
        ...
        try {
            ...
            setError(error.message || "Could not delete selected users.");
        } finally {
    ...
    const handleSaveNewUser = async (newUser: Omit<AdminUser, 'id' | 'joinedDate' | 'totalInvested' | 'totalReturns' | 'ytdReturnPercent' | 'dayChange' | 'dayChangePercent' | 'installments'>) => {
        try {
            ...
            // Re-throw for modal to catch and display
            throw error; 
        }
    }
                    ...
                    <div className="space-x-2">
                        {selectedUsers.size > 0 && (
                            <button
                                ...
                                className="px-3 py-2 text-sm font-semibold text-white bg-rose-600 hover:bg-rose-700 rounded-lg disabled:bg-slate-600 disabled:cursor-wait transition-colors"
                            >
                        ...
                        <button 
                            ...
                            disabled={isSubmitting}
                            className="px-3 py-2 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg disabled:bg-slate-600 disabled:cursor-wait transition-colors"
                        >
                               ...
                               <tr key={user.id} className={`${selectedUsers.has(user.id) ? 'bg-slate-700/50' : ''} hover:bg-slate-700/50 transition-colors`}>
                                   ...
                                   <td className="px-6 py-4 whitespace-nowrap text-sm text-right">
                                       <button 
                                            ...
                                            disabled={isSubmitting}
                                            className="text-rose-500 hover:text-rose-400 font-semibold disabled:text-slate-500 disabled:cursor-wait"
                                        >
                                            Delete
                                       </button>
                                   </td>
